...
EVIDO.AI는 사용자가 업로드한 문서를 기반으로 질문에 답변하고, 답변의 근거가 되는 문서 일부를 함께 제공하는 문서 기반 AI 검색 및 질의응답 서비스입니다.

기존의 일반적인 AI 챗봇은 답변의 출처가 불분명하거나, 사용자의 실제 문서 내용을 정확히 반영하지 못하는 한계가 있습니다. EVIDO는 사용자의 문서를 워크스페이스 단위로 관리하고, 문서 내용을 임베딩하여 벡터 검색을 수행한 뒤, 검색된 문서 조각을 근거로 답변을 생성합니다.

또한 기업 환경에서 중요한 내부 자료 보호를 고려하여 설계했습니다. 업무 매뉴얼, 보고서, 정책 문서와 같은 기업 내부 자료는 외부로 전달되는 것에 부담이 있기 때문에, EVIDO는 LLM 호출부를 특정 외부 API에 종속되지 않도록 분리했습니다. 현재는 Grok API를 활용해 답변을 생성하지만, 향후 GPU 서버나 충분한 연산 환경이 제공될 경우 내부 서버에 오픈소스 LLM을 배포하여 외부 API 없이도 동작할 수 있도록 설계했습니다.

이를 위해 문서 관리, 벡터 검색, 대화 관리, LLM 답변 생성 흐름을 분리하고, LLM 호출 영역은 어댑터 형태로 구성하여 모델 교체가 가능하도록 했습니다. 따라서 외부 API 기반 구조에서 시작하더라도, 기업 환경에 따라 내부망 기반 LLM 또는 온프레미스 LLM으로 전환할 수 있습니다.

이를 통해 사용자는 긴 문서를 직접 읽지 않아도 필요한 정보를 빠르게 찾을 수 있으며, AI 답변이 어떤 문서 내용을 근거로 생성되었는지 확인할 수 있습니다. 동시에 기업이나 조직은 내부 자료를 기반으로 한 AI 검색 시스템을 보다 유연하고 안전하게 운영할 수 있습니다.
...
| 4차 스프린트 | 2026.05.01 ~ 2026.05.30 | 기억 메모리 기반 답변 설계 | GitHub Actions 전환, Blue-Green 배포, S3 저장소 전환, 오케스트레이션( grok 기반 질문 분기 ) |
| 5차 스프린트 예정 | 2026.06 이후 | 서비스 안정화 및 확장 | S3 저장소 전환, OCR, 답변 캐싱, MSA 전환 검토 |

---

## 2. 프로젝트 목적

1. 사용자가 보유한 문서를 AI가 이해할 수 있도록 텍스트 추출, 분할, 임베딩 과정을 거쳐 검색 가능한 형태로 변환
2. 업로드된 문서 내용을 기반으로 사용자의 질문에 답변하여 일반 AI 챗봇보다 문서 맥락에 맞는 응답 제공
3. 답변과 함께 근거 문서 조각을 제공하여 AI 답변의 출처와 신뢰성을 확인할 수 있도록 지원
4. 워크스페이스 단위로 문서를 관리하여 개인, 팀, 조직별 독립적인 문서 활용 환경 제공
5. 기업 내부 자료 보호를 고려하여 문서 저장, 검색, 답변 생성 흐름을 분리하고 외부 API 의존도를 낮출 수 있는 구조 설계
6. LLM 호출부를 어댑터 형태로 분리하여 Grok API, OpenAI API, 내부 오픈소스 LLM 등 다양한 모델로 교체 가능한 구조 구현
7. 향후 GPU 서버 등 충분한 연산 환경이 제공될 경우 외부 LLM API 없이 내부 인프라에서 답변 생성을 처리할 수 있도록 확장
8. 사내 지식 검색, 업무 매뉴얼 질의응답, 문서 요약, 보고서 생성, 업무 자동화 기능으로 확장 가능한 기반 마련
---
